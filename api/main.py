...
# Additive schema changes that Base.metadata.create_all cannot apply to
# pre-existing tables (it only creates missing tables, never missing columns).
# Each statement is safe to re-run; failures (column already exists, SQLite
# not supporting DROP NOT NULL, etc.) are ignored.
SCHEMA_SYNC_STATEMENTS = [
    ...
    # Safe Harbor: replace the full-date column with a year-only column.
    # migrate_truncate_original_dates() below backfills and then destroys the
    # month/day values; this statement only creates the destination column.
...
def migrate_truncate_original_dates(engine) -> None:
    ...
    source FHIR resource, which contradicted the Safe Harbor claim made
    elsewhere in the product. This backfills original_year from it, then
    destroys the month/day values.

    Idempotent and safe to run on every boot. Ordering matters: the year is
    copied before anything is destroyed, and the column is only dropped after
    the values in it have already been nulled, so an interrupted run can never
    lose the year while leaving month/day behind.
    """
    with engine.begin() as conn:
        ...
        # 2. Destroy month/day. This is the step that actually satisfies the
        #    privacy requirement; the DROP below is cleanup.
        conn.execute(text(
            "UPDATE extracted_medical_data SET original_date = NULL "
            "WHERE original_date IS NOT NULL"
        ))

    # 3. Drop the column. Requires SQLite >= 3.35; if it fails the values are
    ...
    try:
        ...
        audit_logger.info("MIGRATION original_date dropped")
    except Exception as exc:
        audit_logger.warning(
            "MIGRATION original_date nulled but not dropped: %s", exc
        )
...
class AddStudySiteRequest(BaseModel):
    institution_id: str
...
class StudySiteResponse(BaseModel):
    ...
    city: Optional[str]
    state: Optional[str]
    documents: List[RegulatorySubmissionResponse]
...
# Authorization
#
# One rule underpins all of this: a JWT proves identity, never privilege. The
# "type" claim is read only to build the token; it is never consulted when
# deciding what a request may do. Every dependency below resolves the user row
# and reads the role, active flag and approval state from the database, so a
# deactivated account, a downgraded role or a revoked approval takes effect on
# the very next request rather than when the token happens to expire.
# ---------------------------------------------------------------------------
...
def current_user(
    ...
    """The single DB-backed dependency for every protected endpoint.

    Requires a token that resolves to a user row that still exists and is
    still active. Bare require_auth only validated the signature, so a token
    belonging to a deleted or deactivated account stayed usable until expiry.
    """
...
# Kept as an alias so existing handlers that ask for the row keep working.
...
def current_user_token(user: User = Depends(current_user)) -> Dict:
    """Identity-only gate, DB-backed, for handlers written against the dict."""
    return {"sub": str(user.id), "type": user.user_type}


def require_role(*allowed_roles: str):
    """Role gate for handlers that want the raw token payload.

    The role is taken from the database row, not the token claim.
    """
    def _check_role(user: User = Depends(current_user)) -> Dict:
...
def require_role_record(*allowed_roles: str):
    """Role gate for handlers that want the User row."""
...
def require_approved_researcher(user: User = Depends(current_user)) -> User:
    ...
    Gates every cohort, researcher, study, regulatory, analytics,
    collaboration and extraction route. All three conditions are separate:
    a verified email proves only control of a mailbox, so approval is a
    distinct human decision recorded on the row by api/manage.py.
    """
    ...
    if user.researcher_approved_at is None:
        raise HTTPException(
            status_code=403,
            detail=(
                "Your researcher account is pending approval. "
                "Research features unlock once an administrator approves it."
            ),
        )
    return user
...
def require_researcher_token(user: User = Depends(require_approved_researcher)) -> Dict:
    """Approved-researcher gate for handlers written against the token dict."""
    return {"sub": str(user.id), "type": user.user_type}


def require_institution_user(user: User = Depends(current_user)) -> User:
    """Institution or admin, and institution users must be scoped to one.

    An institution account with no institution_id could otherwise read and
    write records across every institution.
    """
    if user.user_type not in ("institution", "admin"):
        raise HTTPException(
            status_code=403,
            detail="Access denied. Required role: institution, admin",
        )
...
def require_patient(user: User = Depends(current_user)) -> User:
    ...
    if user.user_type != "patient":
        raise HTTPException(
            status_code=403,
            detail="Access denied. Required role: patient",
        )
    return user


def require_patient_token(user: User = Depends(require_patient)) -> Dict:
    """Patient-only gate for handlers written against the token dict."""
    return {"sub": str(user.id), "type": user.user_type}
...
def process_extraction_job(db: Session, job: ExtractionJob, study: Study, requester_user_id: str) -> None:
    ...
    if patient_ids:
        records = db.query(ExtractedMedicalData).filter(
            ...
            ExtractedMedicalData.original_year,
...
def _parse_fhir_year(value: Any) -> Optional[int]:
    ...
    source month and day. Only the year is returned now, and the month/day
    are never carried past this function.
    """
...
async def submit_regulatory(
    ...
    study = require_study_access(db, request.study_id, token_data["sub"])
...
async def approve_regulatory(
    submission_id: str,
    token_data: Dict = Depends(require_role("admin", "institution")),
    db: Session = Depends(get_db)
):
    """Approve a regulatory submission (admin/institution reviewers only)"""
...
async def submit_draft_regulatory(
    ...
    if not submission.study_id:
        raise HTTPException(status_code=400, detail="Institution-level agreements are managed by the institution")

    study = db.query(Study).filter(Study.id == submission.study_id).first()
    if not study or study.user_id != token_data["sub"]:
        raise HTTPException(status_code=403, detail="Only the study PI can submit regulatory documents")
    ...
    return {
        ...
        "submission_id": str(submission.id),
        "status": submission.status,
        "message": f"{submission.document_type.replace('_', ' ').title()} submitted for review.",
    }


# ============== Multi-Site Study Endpoints ==============

def build_site_documents(db: Session, study_id: str) -> Dict[str, List[RegulatorySubmission]]:
    """Group a study's regulatory submissions by institution (None key = central)"""
    submissions = db.query(RegulatorySubmission).filter(
        RegulatorySubmission.study_id == study_id
    ).all()
    grouped: Dict[str, List[RegulatorySubmission]] = {}
    for sub in submissions:
        grouped.setdefault(sub.institution_id, []).append(sub)
    return grouped


def submission_to_response(db: Session, sub: RegulatorySubmission) -> RegulatorySubmissionResponse:
    inst_name = None
    if sub.institution_id:
        inst = db.query(Institution).filter(Institution.id == sub.institution_id).first()
        inst_name = inst.name if inst else None
    return RegulatorySubmissionResponse(
        id=str(sub.id),
        document_type=sub.document_type,
        status=sub.status,
        protocol_number=sub.protocol_number,
...
@app.post("/api/researcher/studies/{study_id}/sites", response_model=StudySiteResponse)
async def add_study_site(
    ...
    Creates the site's reliance agreement and DUA as drafts, and ensures a
    central IRB protocol submission exists for the study."""
    study = db.query(Study).filter(Study.id == study_id).first()
    if not study:
        raise HTTPException(status_code=404, detail="Study not found")

    if study.user_id != token_data["sub"]:
        raise HTTPException(status_code=403, detail="Only the study PI can add sites")
    ...
    existing = db.query(RegulatorySubmission).filter(
        RegulatorySubmission.study_id == study_id,
        RegulatorySubmission.institution_id == request.institution_id,
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail="This institution is already a site on this study")

    # Ensure the study has a central (single IRB of record) protocol submission
    central_irb = db.query(RegulatorySubmission).filter(
        RegulatorySubmission.study_id == study_id,
        RegulatorySubmission.document_type == "irb_protocol",
        RegulatorySubmission.institution_id == None,
    ).first()
    if not central_irb:
        db.add(RegulatorySubmission(
            study_id=study_id,
            institution_id=None,
    ...
    site_docs = [
        ...
        for doc_type in ("reliance_agreement", "dua")
    ]
    for doc in site_docs:
        db.add(doc)
    db.commit()

    return StudySiteResponse(
        institution_id=str(institution.id),
        institution_name=institution.name,
        city=institution.city,
        state=institution.state,
        documents=[submission_to_response(db, d) for d in site_docs],
    )
...
async def get_study_sites(
    ...
    their regulatory document statuses (PI or collaborator only)"""
    require_study_access(db, study_id, token_data["sub"])

    grouped = build_site_documents(db, study_id)

    central = [submission_to_response(db, s) for s in grouped.pop(None, [])]

    sites = []
    for institution_id, docs in grouped.items():
        ...
        sites.append({
            ...
            "state": institution.state if institution else None,
            "documents": [submission_to_response(db, d) for d in docs],
        })

    return {"central": central, "sites": sites}
...
async def get_researcher_collaborations(
    ...
    collaborations = db.query(StudyCollaborator).filter(
        StudyCollaborator.user_id == token_data["sub"],
        StudyCollaborator.status == "accepted",
    ).order_by(StudyCollaborator.created_at.desc()).all()

    result = []
    for collab in collaborations:
        study = db.query(Study).filter(Study.id == collab.study_id).first()
        if not study:
            continue
        pi = db.query(User).filter(User.id == study.user_id).first()
        site_count = db.query(
            func.count(func.distinct(RegulatorySubmission.institution_id))
        ).filter(
            RegulatorySubmission.study_id == study.id,
            RegulatorySubmission.institution_id != None,
        ).scalar() or 0
        result.append(CollaborationResponse(
            ...
            description=study.description,
            pi_name=pi.name if pi else study.principal_investigator,
            pi_organization=pi.organization if pi else None,
            my_role=collab.role,
            status=study.status,
...
async def get_institution_profile(
    ...
    if not user.institution_id:
        # User is not linked to an Institution row yet; show what we know
...
async def get_institution_agreements(
    ...
    submissions = db.query(RegulatorySubmission).filter(
        ...
        RegulatorySubmission.document_type != "irb_protocol",
    ).order_by(RegulatorySubmission.created_at.desc()).all()

    result = []
    for s in submissions:
        study = db.query(Study).filter(Study.id == s.study_id).first() if s.study_id else None
        result.append(InstitutionAgreementResponse(
            id=s.id,
            document_type=s.document_type,
            name=study.name if study else (s.notes or s.protocol_number),
            status=s.status,
            counterparty=study.principal_investigator if study else s.notes,
...
async def get_institution_irb_protocols(
    ...
    for s in submissions:
        study = db.query(Study).filter(Study.id == s.study_id).first() if s.study_id else None
        result.append(InstitutionIRBResponse(
            id=s.id,
            protocol_number=s.protocol_number,
            name=study.name if study else (s.notes or f"Protocol {s.protocol_number}"),
            status=s.status,
