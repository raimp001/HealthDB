...
class TestStudyObjectLevelAccess:
    def test_cannot_read_another_researchers_study(self, client, register, make_user):
        ...
        cross-user exposure rather than a missing role. This must never skip:
        a skipped authorization check is indistinguishable from an absent one.
        """
        owner_headers, _ = make_user("owner@example.com", role="researcher",
                                     verified=True, approved=True)
        intruder_headers, _ = make_user("intruder@example.com", role="researcher",
                                        verified=True, approved=True)
