...
| `researcher` | Signup, then `python -m api.manage approve-researcher` | Needs **active + verified + approved**; all three are separate |
...
| `GET` | `/api/openapi.json` | anonymous |
...
| `POST` | `/api/regulatory/{submission_id}/approve` | role-gated (see handler) |
...
| `GET` | `/api/studies/available` | patient |
| `POST` | `/api/studies/{study_id}/join` | patient |
| `POST` | `/api/studies/{study_id}/leave` | patient |
...
Each was reviewed individually: authentication entry points, reference and
catalogue data with no subject, and FastAPI's own schema and documentation
endpoints. `/api/auth/register` additionally refuses unless the operator has
opened pilot enrollment.
